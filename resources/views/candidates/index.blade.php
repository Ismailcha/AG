<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('candidats.index') }}
    @endsection
                                ...
                                <div class="card">
                                    <div class="card-body d-flex flex-center flex-column py-9 px-5">
                                        <div class="symbol symbol-65px symbol-circle mb-5">
                                            <img style="50px" src="{{ asset('storage/' . $candidate->avatar) }}"
                                                alt="image" />
                                            <div
                                        ...
                                        <button class="btn btn-sm btn-light-primary btn-flex btn-center"
                                            ...
                                            data-bs-target="#pdfModal{{ $candidate->id }}">
                                            <span class="indicator-label">Afficher le CV</span>
                                        </button>

                                        <!-- Create the modal structure with a unique ID -->
                                        <div class="modal fade" id="pdfModal{{ $candidate->id }}" tabindex="-1"
                                            aria-labelledby="pdfModalLabel{{ $candidate->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="pdfModalLabel{{ $candidate->id }}">
                                                            CV de {{ $candidate->nom }} {{ $candidate->prenom }}</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                            aria-label="Close"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        <!-- Use a unique ID for the iframe element -->
                                                        <iframe id="pdfViewer{{ $candidate->id }}"
                                                            src="{{ asset('storage/' . $candidate->resume_path) }}"
